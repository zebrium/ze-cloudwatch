...
const getMetaData = (eventData, config) => {
    ...
    // logStream is the host name followed by dot numeric e.g myhostname.0
    var hostname = config.ze_hostname || eventData.logStream.split( '.' )[0];
    // logGroup is a path where the logbasename (lbn) is the last element in the path
    var lbn = eventData.logGroup.replace( /^.*[\\\/]/, '' );
