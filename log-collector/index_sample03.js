...
const getMetaData = (eventData, config) => {
    ...
    var hostname = eventData.logGroup;
    var lbn = eventData.logGroup;
    const lambda_match = eventData.logGroup.match(/^\/aws\/lambda\//);
    if (lambda_match) {
        // Expect the following:
        //    /aws/lambda/my-prod-account-sf-psql
        //    /aws/lambda/my-prod-flushs3
        hostname = eventData.logGroup.replace(/^.*[\\\/]/, '')
        const fields = hostname.split("-");
        if (fields.length > 3) {
            lbn = fields.slice(-2).join("-");
        } else if (fields.length <= 3) {
            lbn = fields[fields.length - 1];
        }
    } else {
        // Check format used by aws:
        // /aws/rds/instance/my-staging-database/postgresql
         const aws_format_match = eventData.logGroup.match(/^\/aws\/[0-9a-zA-Z]+\/instance\//);
         if (aws_format_match) {
             const fields = eventData.logGroup.split("/");
             hostname = fields[fields.length - 2];
             lbn = fields[fields.length - 1]
         } else if (eventData.logGroup.startsWith("/") == false ) {
             // This is log_group name without "/aws" prefix
             hostname = config.ze_hostname || "zebrium-poc";
             lbn = eventData.logGroup.split('.')[0];
         }
    }
